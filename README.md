# 2021
Non-formal Project of 2021

* [ThoughtWorks西安面试家庭作业](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/ConferenceTrackManagement/src) 
* [EFCore分库、分表、多租户的实现](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/EF.Sharding) 
* [一个构建于ASP.NET Core上的RPC框架](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/FakeRPC)
* [gRPC在拦截器、重试、健康检查、RESTful支持等方面的探索](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/GRPC.Logging)
* [Kafkax消息中间件的学习和探索](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src)
* [EFCore实体命名约束库的学习](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/EF.Naming)
* [Python多线程/进程学习](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/multiple-threading)
* [2020年全年微博热搜数据分析](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/resou-analyse)
* [ASP.NET Core + Envoy实现反向代理](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/EnvoyGateway)
* [ASP.NET Core + Envoy实现负载均衡](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/EnvoyLB)
* [gRPC + Envoy实现RPC服务的RESTful化](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/EnvoyGrpc)
* [gRPC + Envoy实现微服务的监控预警](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/EnvoyMonitor)
* [ASP.NET Core + Envoy实现微服务的认证](https://hub.fastgit.org/Regularly-Archive/2021/tree/master/src/EnvoyJwt)


